...
每个 `PetAction` 对应一个 128×128 透明 GIF，并由 Manifest 描述循环、fallback、锚点和 hash。状态机切换动作时切换到对应 GIF 并从首帧重新播放；运行时可在加载阶段预解码为缓存纹理以满足 PixiJS 性能，但正式交付格式仍是 GIF。详细格式见 `11-asset-pipeline-contract.md`。

非循环 GIF 播放结束后进入其 `fallbackAction`；缺失、损坏或解码失败时直接使用 fallback，最终回到 `IDLE`。本地与好友端只同步 `PetAction`、强度和序号，不同步 GIF 帧位置。

### 5.1 登录页装饰动画位

- 登录页右侧固定展示“金币砸脑壳”循环动画：金币从上方落下、轻砸水豚头部并弹开，角色继续坐在桌前敲键盘。
- 该动画属于本地 UI 装饰，不进入 `PetAction` 联机枚举，不向好友广播，也不依赖活动识别。
- 正式客户端加载 `ui/login-coin-hit` 的 128×128 透明循环 GIF + Manifest；缺失时使用静态关键帧。

## 6. 渲染与像素风约束

- 使用整数倍缩放与 nearest-neighbor 采样；
- 所有角色动作逻辑画布固定为 128×128；GIF 帧不得改变逻辑画布尺寸。
...
- 帽子堆叠超出窗口时直接提供纵向滚动视口，不自动缩小角色或帽子塔，不能改变装备数据。
...
GIF 必须在动作首次使用前完成解码或进入可预测的加载态；同一 `assetVersion + action` 只保留一份共享帧缓存，切换动作不得重复解码。
...
11. 每次动作切换会从目标 GIF 首帧开始；非循环动作结束按 Manifest 回退，损坏 GIF 不会阻断任务功能。
12. 超高帽子塔不缩小，纵向滚动可以到达全部已装备帽子。

## 11. 已确认项

`[已确认: PET-001][2026-08-29]` 同时提供独立透明桌宠窗口和设置主窗口。
`[已确认: PET-002][2026-08-29]` MVP 不提供拍键盘音效。
`[已确认: PET-MAC-001][2026-08-29]` 设置主窗口关闭后隐藏 Dock 图标，重新打开设置时恢复。
`[已确认: PET-MAC-002][2026-08-29]` 普通 Spaces 显示；演示、视频和屏幕共享类全屏默认隐藏。
`[已确认: HAT-STACK-001][2026-08-29]` 超出高度后直接纵向滚动，不自动缩小；静态帽子层可缓存。
