...
- Retina 内建屏、非 Retina 外接屏及常见 macOS 显示缩放下像素边缘不模糊；渲染器必须区分逻辑像素与设备像素；
- 默认帧率可为 12–24fps，按资源 Manifest；
- 透明桌宠窗口点击穿透策略可切换，设置或拖动时暂时接收点击；
- 桌宠窗口位置以当前显示器 `workArea` 为准，不假定 Dock 位于底部；Dock 位置、自动隐藏或显示器变化后必须重新约束窗口位置；
- 菜单栏入口负责显示/隐藏桌宠、打开今日任务和设置；不得用网页模拟系统菜单栏图标；
- Spaces、全屏应用和 Stage Manager 下的可见策略必须由统一窗口控制器执行，不允许 renderer 自行猜测窗口层级；
...
`[待确认: PET-MAC-001]` 只有菜单栏与桌宠可见时是否隐藏 Dock 图标。建议设置主窗口关闭后隐藏 Dock 图标，重新打开设置时恢复。  
`[待确认: PET-MAC-002]` 桌宠是否覆盖 macOS 全屏应用。建议普通 Spaces 显示，全屏演示、视频与屏幕共享时默认隐藏。  
