...
| 首发区域 | 仅中国大陆；工作日与结算固定使用 `Asia/Shanghai` |
...
| 屏幕视觉识别 | Screen Recording | 产品开关默认开启，等待系统授权 | 不获取屏幕像素，不调用视觉接口 |
...
截图识别虽然产品开关默认开启，仍不能把本地 `enabled=true` 当作 macOS 已授权。首次权限引导必须先说明打码、在线模型和无留存边界，再触发系统授权；拒绝后保持降级且不循环请求。
...
`[已确认: PLATFORM-UPDATE-001][2026-08-29]` 自动更新文件托管使用 S3 兼容对象存储，支持分阶段发布和快速撤回更新元数据。
`[已确认: PET-MAC-001][2026-08-29]` 设置主窗口关闭后隐藏 Dock 图标，打开设置时恢复。
`[已确认: PET-MAC-002][2026-08-29]` 普通 Spaces 显示；演示、视频和屏幕共享类全屏默认隐藏。
