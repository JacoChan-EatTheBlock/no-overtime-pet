...
| 屏幕截图视觉 | 经 Screen Recording 授权后获取、降采样并打码的当前屏幕 | 单独授权 | `LOCAL_RAW`，默认不留存 |
...
- 屏幕像素采集只在“高准确模式”中使用 Screen Recording；系统权限未授予时不得反复弹窗或绕过系统设置。
...
`[待确认: ACTIVITY-003]` 经 Accessibility 授权后，原始窗口标题是否允许短暂进入本地内存规则。建议允许但默认不落盘。  
