...
让已互加好友且允许展示的用户在一个轻量像素场景里排排坐，实时看到彼此正在工作、开会、摸鱼、离开或已经跑路，以及对应桌宠动作。
...
- 删除好友后服务端推送撤销事件，并从双方房间移除。
- 全局活动广播关闭或单个好友被设为“不对其展示”时，只撤销当前用户到目标好友的活动投影，好友关系保持 `ACCEPTED`，反向投影不受影响。
- `showFriendPetsOnDesktop=false` 时，客户端不订阅或不渲染好友桌宠；该设置不得反向关闭自己的活动广播。
...
4. 乱序事件不会把角色回滚到旧动作。
5. 删除好友后 3 秒内从双方场景移除。
6. “不对其展示”后 3 秒内仅撤销当前用户到目标好友的投影，不解除好友，也不影响反向投影。
7. 关闭桌面好友桌宠显示后，本机不再渲染好友角色，但自己的广播设置保持不变。
8. 事件抓包不含任务、工资、应用名、URL 或具体按键。
9. 12 个角色同时显示时达到性能预算。
