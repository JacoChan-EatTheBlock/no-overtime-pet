...
- 原子任务不得拆分。
- 可拆分任务按策略最小块拆分，MVP 最小块为 25 分钟。
- 默认使用 50 分钟专注、10 分钟休息；允许用户调整，且不能挤占用户午休。
- 已经过的时间不可安排新块。
...
`[已确认: SCHEDULE-001][2026-08-29]` 未完成任务仍允许按时跑路；记录非胜利但不阻止。
`[已确认: SCHEDULE-002][2026-08-29]` MVP 不接系统日历，只支持手动固定会议。
`[已确认: SCHEDULE-003][2026-08-29]` 默认 50/10，允许用户调整。
`[已确认: SCHEDULE-004][2026-08-29]` 软计划自动优化默认关闭。
