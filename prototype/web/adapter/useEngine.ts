...
import { applyProposal, type AcceptRequest } from '../../src/accept.js'
import { baselineProposal } from '../../src/baseline.js'
import { solveSchedule } from '../../src/scheduler.js'
import { personalMultiplier, type CalibrationSample } from '../../src/calibration.js'
import type {
  Importance,
  ProposalField,
  ScheduleDraft,
  ...
  const analyseTask = useCallback(
    async (task: Task): Promise<AnalysisState> => {
      try {
        ...
        // 网络层失败也不能中断流程 —— 与逻辑层同样的降级承诺（PRD 05 §7）。
        return {
          task,
          proposal: baselineProposal(task.id, task, DEMO_HISTORY, task.revision, nowMs),
          source: 'BASELINE',
          fallbackReason: `CLIENT_${error instanceof Error ? error.message : 'UNKNOWN'}`
        }
      }
    },
    [nowMs]
